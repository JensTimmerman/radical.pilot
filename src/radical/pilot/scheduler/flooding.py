...
.. module:: radical.pilot.scheduler.BackfillingScheduler
   :platform: Unix
   :synopsis: A multi-pilot, flooding scheduler.
...
# -----------------------------------------------------------------------------
# 
class FloodingScheduler(Scheduler):
    """
    
    FloodingScheduler implements a multi-pilot, flooding scheduling
    algorithm. All units which are eligible for scheduling are scheduled over
    all NEW, PENDING, PENDING_ACTIVE and ACTIVE pilots -- where ACTIVE and
    PENDING_ACTIVE pilots (in this order) are preferred and kept full.

    If a unit finishes, or some pilot state changes, and the equlibrium of
    schedule is thus disturbed, then units from non-active pilots are
    redistributed to recover the equilibrium.

    The functionality relies on the ability to change the pilot assigned to
    a unit in the DB like:

      query : _id   = uid
              pilot = pid_old
              state = not executing
      update: pilot = pid_new
              state = pending

    Note that rescheduling may imply re-staging - we thus prefer to reschedule
    units which have not staged anything, or have nothing to stage...
    """
